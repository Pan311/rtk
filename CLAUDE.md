...
### Windows PowerShell (60-80% savings)
```powershell
# Directory/File operations
rtk powershell Get-ChildItem       # Directory listing, compact (70%)
rtk powershell Get-Item file.txt   # File info with truncation
rtk powershell Copy-Item src dst   # Copy confirmation (minimal output)
rtk powershell Move-Item src dst   # Move confirmation (minimal output)
rtk powershell New-Item -Name file # Create item (80% savings)

# Process management
rtk powershell Get-Process         # Process list, truncated to 50 (80%)
rtk powershell Get-Process chrome  # Filter specific processes
rtk powershell Start-Process       # Launch process
rtk powershell Stop-Process        # Kill process

# Services
rtk powershell Get-Service         # Service status, compact (75%)
rtk powershell Get-WinEvent        # Windows events with filtering (75%)

# File content & search
rtk powershell Get-Content file.txt   # Read with head/tail (60%)
rtk powershell Select-String pattern  # Search text (70%)

# Utilities
rtk powershell Test-Path path      # Check path exists
rtk powershell Remove-Item file    # Delete file/directory
rtk powershell Sort-Object         # Sort output
rtk powershell Format-Table        # Format as table
...
| Windows PowerShell | Get-ChildItem, Get-Process, Get-Service, Get-ComputerInfo, Get-Counter | 60-85% |
